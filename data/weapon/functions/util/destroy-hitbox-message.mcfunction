...
execute as @e[type=armor_stand,tag=plane-root,distance=..30] if score @s vp.plane-id = #parts-id vp.reg1 run tag @s add parts-owner
execute as @e[type=armor_stand,tag=plane-hitbox,distance=..30] if score @s vp.plane-id = #parts-id vp.reg1 run tag @s add parts-hitbox

#機体破壊判定
execute as @e[type=armor_stand,tag=plane-root,tag=parts-owner,distance=..30] at @s run function weapon:util/parts-health
execute if entity @e[type=armor_stand,tag=plane-root,tag=parts-owner,tag=destroyed,distance=..30] run title @p[tag=bullet-owner] subtitle {"text":"敵機を撃墜","color":"gold","italic":true}
execute if entity @e[type=armor_stand,tag=plane-root,tag=parts-owner,tag=!destroyed,distance=..30] run title @p[tag=bullet-owner] subtitle {"text":"クリティカルヒット","color":"white","italic":true}
