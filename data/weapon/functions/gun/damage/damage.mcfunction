...
scoreboard players operation @e[tag=hit-weapon,distance=..20,type=!player] vp.reg2 = @s vp.damage
...
execute as @e[tag=hit-weapon,distance=..20,scores={vp.reg1=0},sort=nearest,limit=1] run function weapon:gun/damage/set-kill-mob-message
execute as @e[tag=hit-weapon,distance=..20,scores={vp.reg1=0},tag=enemy-target,sort=nearest,limit=1] run function weapon:gun/damage/set-kill-target-message
execute if entity @e[tag=!entity-nohit,distance=..20,scores={vp.reg1=0}] run title @p[tag=bullet-owner] title {"text":""}
...
execute as @e[tag=hit-weapon,tag=plane-hitbox,tag=!cockpit,distance=..20,scores={vp.reg1=0}] run function weapon:util/destroy-hitbox-message
...
#スコアをHPに反映
execute as @e[tag=hit-weapon,distance=..20,type=!spawner_minecart] store result entity @s Health float 1 run scoreboard players get @s vp.reg1
execute as @e[tag=hit-weapon,distance=..20,type=spawner_minecart] store result entity @s MaxNearbyEntities float 1 run scoreboard players get @s vp.reg1

#破壊音
execute at @e[tag=hit-weapon,distance=..20,scores={vp.reg1=0},tag=enemy-target] run playsound minecraft:entity.generic.explode ambient @a ~ ~ ~ 16 0

#破壊されたスポナーをキル
kill @e[tag=hit-weapon,distance=..20,scores={vp.reg1=0},tag=enemy-target]
