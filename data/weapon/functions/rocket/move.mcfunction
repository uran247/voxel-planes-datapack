...
tp @e[tag=rocket-move-executer,distance=..1,limit=1] ~ ~ ~ facing entity @s
...
#命中していない場合移動予定先へ移動
execute if score #hit-flag vp.reg1 matches 0 run tp @e[tag=rocket-move-executer,distance=..1,limit=1] @s

#命中してた場合命中してたところに移動
execute if score #hit-flag vp.reg1 matches 1.. run tp @e[tag=rocket-move-executer,distance=..1,limit=1] @e[tag=hit-weapon,distance=..26,sort=nearest,limit=1]
