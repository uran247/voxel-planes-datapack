#>  plane-data:j2m3/weapon/model1-20mm
#
# 20mm機銃を使用
#
# @input
...
execute if score #is-bullet vp.reg1 matches 0 unless score #is-tracer vp.reg1 matches 0 run summon minecraft:area_effect_cloud ~ ~ ~ {NoGravity:1b,Tags:[projectile,gun,20mm,gun-init,right1,tracer,tracer-lightblue,entity-nohit,offset-base],Duration:30}
execute if score #is-bullet vp.reg1 matches 0 unless score #is-tracer vp.reg1 matches 0 run summon minecraft:area_effect_cloud ~ ~ ~ {NoGravity:1b,Tags:[projectile,gun,20mm,gun-init,left1,tracer,tracer-lightblue,entity-nohit,offset-base],Duration:30}
...
scoreboard players set @e[tag=gun-init,tag=left1,distance=..5] vp.offsetX 2260
scoreboard players set @e[tag=gun-init,tag=left1,distance=..5] vp.offsetY -350
scoreboard players set @e[tag=gun-init,tag=left1,distance=..5] vp.offsetZ 2700
scoreboard players set @e[tag=gun-init,tag=right1,distance=..5] vp.offsetX -2460
scoreboard players set @e[tag=gun-init,tag=right1,distance=..5] vp.offsetY -350
scoreboard players set @e[tag=gun-init,tag=right1,distance=..5] vp.offsetZ 2650
