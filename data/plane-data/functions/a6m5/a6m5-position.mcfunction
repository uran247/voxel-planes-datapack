...
execute as @s[tag=bombed,scores={reg1=0}] if score @s ammunition3 = @s max-ammunition3 at @s store success score #summoned-bomb reg1 run function plane-data:a6m5/summon/a6m5-bomb
execute if score #summoned-bomb reg1 matches 1.. as @e[tag=a6m5,tag=!dropping,tag=plane-bomb,distance=..10] if score @s plane-id = #plane-id reg1 run tag @s add target-parts
