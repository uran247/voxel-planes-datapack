#> plane-data:g4m1/weapon/rear-gun4
#
# 20mm前部機銃を使用
# 実行者：機体
#
...
#実際に発射するかの判定
scoreboard players operation #is-bullet vp.reg1 = @s vp.ammunition5
...
#弾召喚
execute if score #is-tracer vp.reg1 matches 0 run summon armor_stand ~ ~ ~ {NoGravity:1b,Invisible:1b,Tags:[projectile,gun,20mm,gun-init,right,tracer,tracer-yellow,entity-nohit,offset-base]}
...
scoreboard players set @e[tag=gun-init,distance=..5] vp.damage 35
...
#発射したならreload時間設定
execute if entity @e[tag=gun-init,distance=..20] run scoreboard players set @s vp.w5-cooltime 3
