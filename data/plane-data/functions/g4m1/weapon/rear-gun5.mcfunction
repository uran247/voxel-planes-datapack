#> plane-data:g4m1/weapon/rear-gun5
#
# 7.7mm前部機銃を使用
# 実行者：機体
#
...
#実際に発射するかの判定
scoreboard players operation #is-bullet vp.reg1 = @s vp.ammunition6
scoreboard players operation #is-tracer vp.reg1 = @s vp.ammunition6
...
scoreboard players set @e[tag=gun-init,distance=..5] vp.offsetX 50
scoreboard players set @e[tag=gun-init,distance=..5] vp.offsetY 2000
scoreboard players set @e[tag=gun-init,distance=..5] vp.offsetZ 3500
...
#残弾数減算
scoreboard players remove @s vp.ammunition6 1
