...
execute as @s[tag=bombed,scores={reg1=0}] if score @s ammunition2 = @s max-ammunition2 at @s store success score #summoned-bomb reg1 run function plane-data:ki-44/summon/ki44-bomb
execute if score #summoned-bomb reg1 matches 1.. as @e[tag=ki44,tag=!dropping,tag=plane-bomb,distance=..10] if score @s plane-id = #plane-id reg1 run tag @s add target-parts
