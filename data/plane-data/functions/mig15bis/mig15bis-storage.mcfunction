...
data modify storage voxel-planes:plane mig15bis.plane-data.flight-model.energy-loss-rate set value 0.5
...
#デフォルト機銃データセット
data modify storage voxel-planes:weapon mig15bis.base.23mm set value {data:{name:"23mm",type:gun,size:23mm,max-ammunition:80,current-ammunition:80,max-cooltime:2,current-cooltime:0,max-reload:300,current-reload:0,tracer:"orange",damage:130,speed:8.6f,bullets:[{offset:[0.1d,0d,5d]},{offset:[0.4d,0d,5d]}]}}
data modify storage voxel-planes:weapon mig15bis.base.37mm set value {data:{name:"37mm",type:gun,size:37mm,max-ammunition:40,current-ammunition:40,max-cooltime:4,current-cooltime:0,max-reload:300,current-reload:0,tracer:"lightgreen",damage:230,speed:8.6f,bullets:[{offset:[-0.3d,0d,5d]}]}}

#デフォルト爆弾データセット
#data modify storage voxel-planes:weapon mig15bis.base.bomb.data set value {name:bomb,type:bomb,max-ammunition:0,current-ammunition:0,max-reload:$RELOAD_TIME,current-reload:0,bombs:[]}
#data modify storage voxel-planes:weapon mig15bis.additional.$ADDITIONAL_WEAPOM set value [{kind:$WEPAON_KIND,type:$BOMB_TYPE,offset:[$X,$Y,$Z],damage:$DAMAGE,weight:$WEPAON_WEIGHT,cmd:$WEAPON_CMD}]
