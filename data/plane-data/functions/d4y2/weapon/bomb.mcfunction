...
#スコア付与
scoreboard players operation @e[tag=drop-init,distance=..10] vp.speed = @s vp.speed
scoreboard players operation @e[tag=drop-init,distance=..10] vp.speed /= #1000 vp.Num
scoreboard players set @e[tag=drop-init,distance=..10] vp.age 1200

#角度代入
execute store result entity @e[tag=drop-init,distance=..10,limit=1] Rotation[1] float 0.01 run scoreboard players get @s vp.AngX
execute store result entity @e[tag=drop-init,distance=..10,limit=1] Rotation[0] float 0.01 run scoreboard players get @s vp.AngY
