...
execute store result score @s[tag=rocketted] vp.reg2 if entity @e[tag=target-parts,tag=plane-rocket] 

execute as @s[tag=bombed,scores={vp.reg1=0}] if score @s vp.ammunition2 = @s vp.max-ammo2 at @s store success score #summoned-bomb vp.reg1 run function plane-data:d4y2/summon/d4y2-bomb
execute as @s[tag=rocketted,scores={vp.reg2=0}] if score @s vp.ammunition4 = @s vp.max-ammo4 at @s store success score #summoned-rocket vp.reg1 run function plane-data:d4y2/summon/d4y2-rocket
