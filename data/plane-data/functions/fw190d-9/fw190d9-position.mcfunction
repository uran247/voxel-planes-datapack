#> plane-data:fw190d-9/fw190d9-position
#
# plane-seatの位置を調整する
#
# @input
...
execute as @s[tag=has-bomb,scores={vp.reg1=0}] if score @s vp.ammunition3 = @s vp.max-ammo3 at @s store success score #summoned-bomb vp.reg1 run function plane-data:fw190d-9/summon/fw190d9-bomb
execute if score #summoned-bomb vp.reg1 matches 1.. as @e[tag=fw190d9,tag=plane-bomb,tag=!dropping,distance=..10] if score @s vp.plane-id = #plane-id vp.reg1 run tag @s add target-parts
