#> plane-data:ki-21/weapon/rear-gun2
#
# 7.7mm前部機銃を使用 ki21
# 実行者：機体
#
...
#召喚
execute if score #is-tracer vp.reg1 matches 0 run summon minecraft:armor_stand ~ ~ ~ {NoGravity:1b,Invisible:1b,Tags:[projectile,gun,7p7mm,right1,gun-init,tracer,tracer-lightblue,entity-nohit,offset-base]}
execute if score #is-bullet vp.reg1 matches 0 unless score #is-tracer vp.reg1 matches 0 run summon minecraft:area_effect_cloud ~ ~ ~ {NoGravity:1b,Tags:[projectile,gun,7p7mm,right1,gun-init,tracer,tracer-lightblue,entity-nohit,offset-base],Duration:20}
execute if score #is-tracer vp.reg1 matches 0 run summon minecraft:armor_stand ~ ~ ~ {NoGravity:1b,Invisible:1b,Tags:[projectile,gun,7p7mm,left1,gun-init,tracer,tracer-lightblue,entity-nohit,offset-base]}
execute if score #is-bullet vp.reg1 matches 0 unless score #is-tracer vp.reg1 matches 0 run summon minecraft:area_effect_cloud ~ ~ ~ {NoGravity:1b,Tags:[projectile,gun,7p7mm,left1,gun-init,tracer,tracer-lightblue,entity-nohit,offset-base],Duration:20}
...
scoreboard players set @e[tag=gun-init,tag=right1,distance=..5] vp.offsetX -150
scoreboard players set @e[tag=gun-init,tag=left1,distance=..5] vp.offsetX 0
scoreboard players set @e[tag=gun-init,distance=..5] vp.offsetY 2090
scoreboard players set @e[tag=gun-init,distance=..5] vp.offsetZ -1220
...
#向きをターゲット方向に向ける
execute as @e[tag=gun-init,distance=..20,limit=2] at @s run tp @s ~ ~ ~ facing entity @e[tag=rear-gun-target,distance=..32,limit=1]
...
execute as @e[tag=gun-init,distance=..20] run scoreboard players operation #rand vp.reg1 %= #300 vp.Num
