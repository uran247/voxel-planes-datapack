#> plane-data:ki-21/weapon/rear-gun1
#
# 7.7mm前部機銃を使用 ki21
# 実行者：機体
#
...
execute if score #is-bullet vp.reg1 matches 0 unless score #is-tracer vp.reg1 matches 0 run summon minecraft:area_effect_cloud ~ ~ ~ {NoGravity:1b,Tags:[projectile,gun,7p7mm,gun-init,tracer,tracer-lightblue,entity-nohit,offset-base],Duration:20}
...
scoreboard players set @e[tag=gun-init,distance=..5] vp.offsetY -600
scoreboard players set @e[tag=gun-init,distance=..5] vp.offsetZ 10000
...
execute as @e[tag=gun-init,distance=..20] run scoreboard players remove #rand vp.reg1 150
