#> plane-data:ki-61/summon/ki61-bomb
#
# 爆弾召喚
#
# @input
...
#### 250kg爆弾2個目召喚 ####
execute if entity @s[tag=250kg] run summon armor_stand ~ ~ ~ {Tags:[ki61-bomb-left,"ki61",weapon-init,plane,has-offset,has-model,plane-bomb,plane-parts,entity-nohit,250kg,bomb-normal],NoGravity:1b,Invisible:1,HandItems:[{id:"minecraft:diamond_sword",Count:1b,tag:{CustomModelData:77,Unbreakable:1}},{}],Pose:{RightArm:[0f,0f,0f]},DisabledSlots:256}
scoreboard players set @e[tag=weapon-init] vp.offsetX 2600
scoreboard players set @e[tag=weapon-init] vp.offsetY -500
scoreboard players set @e[tag=weapon-init] vp.offsetZ 500
scoreboard players set @e[tag=weapon-init] vp.damage 1250
...
#残弾スコア付与
scoreboard players set @s[tag=250kg] vp.ammunition3 2
