...
#
# @within function plane-data:ki-61/ki61-weapon-manager
...
execute if score #is-bullet vp.reg1 matches 0 unless score #is-tracer vp.reg1 matches 0 run summon area_effect_cloud ~ ~ ~ {Tags:[projectile,gun,20mm,gun-init,right1,tracer-yellow,entity-nohit,offset-base],Duration:30}
execute if score #is-bullet vp.reg1 matches 0 unless score #is-tracer vp.reg1 matches 0 run summon area_effect_cloud ~ ~ ~ {Tags:[projectile,gun,20mm,gun-init,left1,tracer-yellow,entity-nohit,offset-base],Duration:30}

#スコア付与
scoreboard players set @e[tag=gun-init,distance=..5] vp.speed 90
execute unless entity @s[tag=mg151] run scoreboard players set @e[tag=gun-init,distance=..5] vp.damage 60
execute if entity @s[tag=mg151] run scoreboard players set @e[tag=gun-init,distance=..5] vp.damage 80
...
scoreboard players set @e[tag=gun-init,tag=left1,distance=..5] vp.offsetX 2200
scoreboard players set @e[tag=gun-init,tag=left1,distance=..5] vp.offsetY -180
scoreboard players set @e[tag=gun-init,tag=left1,distance=..5] vp.offsetZ 2200
scoreboard players set @e[tag=gun-init,tag=right1,distance=..5] vp.offsetX -2280
scoreboard players set @e[tag=gun-init,tag=right1,distance=..5] vp.offsetY -180
...
execute unless entity @s[tag=mg151] run playsound minecraft:weapon.heavy-muchingun.fire master @a ~ ~ ~ 1 1 1
