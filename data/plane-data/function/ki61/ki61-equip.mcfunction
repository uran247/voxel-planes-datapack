...
# @within
#   plane-data:ki61/ki61-equip
#   plane-data:ki61/ki61-weapon-manager
...
execute store success score #mg151 vp.reg1 if entity @e[type=minecraft:donkey,tag=target-parts,nbt={Items:[{components:{"minecraft:custom_data":{item-type:mg151}}}]},distance=..30]
...
#装備済み爆弾削除
execute unless score #250kg vp.reg1 matches 1.. run kill @e[tag=target-parts,tag=plane-bomb,distance=..32]
