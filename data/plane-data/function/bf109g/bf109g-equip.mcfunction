...
#装備品チェック
execute store success score #250kg vp.reg1 if entity @e[type=minecraft:donkey,tag=target-parts,nbt={Items:[{components:{"minecraft:custom_data":{item-type:250kg-bomb}}}]},distance=..30]
#execute store success score #aa-rocket vp.reg1 if entity @e[type=minecraft:donkey,tag=target-parts,nbt={Items:[{components:{"minecraft:custom_data":{item-type:ag-rocket}}}]},distance=..30]
execute store success score #gunpod vp.reg1 if entity @e[type=minecraft:donkey,tag=target-parts,nbt={Items:[{components:{"minecraft:custom_data":{item-type:mg151}}}]},distance=..30]
...
#gunpod装着
execute if score #gunpod vp.reg1 matches 1.. unless entity @e[type=item_display,tag=target-parts,tag=plane-gunpod,distance=..32] run summon item_display ~ ~ ~ {Tags:["plane-gunpod","bf109g",plane-init,plane,has-model,entity-nohit,target-parts,position-processed],NoGravity:1b,item:{id:"minecraft:diamond_sword",Count:1b,components:{"minecraft:custom_model_data":143}},interpolation_duration:0,view_range:4.0f,transformation:{right_rotation:{angle:0f,axis:[0f,1f,0f]},left_rotation:{angle:0f,axis:[0f,0f,1f]},scale:[1f,1f,1f],translation:[0f,-0.5f,0.7f]},item_display:"thirdperson_righthand"}
...
execute if score #gunpod vp.reg1 matches 1.. as @e[type=item_display,tag=plane-init,tag=plane-gunpod,distance=..1] store result entity @s attributes[{id:"minecraft:generic.movement_speed"}].base double 1 run scoreboard players get @s vp.plane-id
