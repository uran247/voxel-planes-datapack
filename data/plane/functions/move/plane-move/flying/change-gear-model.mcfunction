...
execute if entity @s[tag=flying-model,tag=!stall] if score @s vp.speed <= @s vp.gear-ret as @e[tag=target-parts,tag=model-changeable] store result entity @s HandItems[0].tag.CustomModelData int 1 run scoreboard players get @s vp.rolling-cmd
