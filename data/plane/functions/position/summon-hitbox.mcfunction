...
#cockpit
execute if score #difficulty vp.reg1 matches 1.. if data storage oh_my_dat: _[-4][-4][-4][-4][-4][-4][-4][-4].status.cockpit unless data storage oh_my_dat: _[-4][-4][-4][-4][-4][-4][-4][-4].status.cockpit{hp:0d} unless entity @e[tag=plane-hitbox,tag=cockpit,tag=target-parts,distance=..30] run summon armor_stand ~ ~ ~ {Tags:[plane-init,target-parts,plane,plane-hitbox,cockpit,has-offset,offset-base],NoGravity:1b,Invisible:1b,Marker:1b,Health:1000f,Attributes:[{Name:"generic.max_health",Base:1000d}]}
...
execute as @e[type=armor_stand,tag=plane-init,tag=cockpit,distance=..1,limit=1] run function plane:position/util/set-hitbox-data
