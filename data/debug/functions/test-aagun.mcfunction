...
#-360 - 360に補正
execute as @e[tag=aagun-init,distance=..5,scores={vp.reg1=..-360}] run scoreboard players add @s vp.reg1 360
execute as @e[tag=aagun-init,distance=..5,scores={vp.reg1=360..}] run scoreboard players remove @s vp.reg1 360
#代入
...
#発射したならreload時間設定
execute if entity @e[tag=aagun-init,distance=..20] run scoreboard players set @s vp.w1-reload 100
